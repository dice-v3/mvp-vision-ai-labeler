...
def main():
    ...
    print(f"  Labeler DB Host: {settings.LABELER_DB_HOST}")
    print(f"  Labeler DB Port: {settings.LABELER_DB_PORT}")
    print(f"  Labeler DB Name: {settings.LABELER_DB_NAME}")
    print(f"  Labeler DB User: {settings.LABELER_DB_USER}")
    print()
    ...
    print("=" * 60)
    if tables_ok:
        print_status("Database initialization completed successfully!", "OK")
    else:
        print_status("Database initialized but some tables are missing", "WARN")
        print_status("This may be expected if migrations are still in progress", "INFO")
    print("=" * 60)
