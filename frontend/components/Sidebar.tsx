...
interface SidebarProps {
  ...
  onLogin?: () => void; // For guest users
  onLogout: () => void;
  onInvitationsClick?: () => void; // Phase 8.2
  onAdminDatasetsClick?: () => void; // Phase 15 - Admin menu
  onAdminAuditLogsClick?: () => void; // Phase 15 - Admin menu
  onAdminStatsClick?: () => void; // Phase 15 - Admin menu
}
...
export default function Sidebar({
  ...
  onLogout,
  onInvitationsClick,
  onAdminDatasetsClick,
  onAdminAuditLogsClick,
  onAdminStatsClick,
}: SidebarProps) {
  const { user } = useAuth();
    ...
    <div className="w-64 h-screen bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="space-y-1">
            {datasets.length === 0 ? (
              <p className="text-sm text-gray-400 py-4 text-center">
                {user ? '데이터셋이 없습니다' : '로그인하여 데이터셋을 확인하세요'}
              </p>
            ) : (
      ...
      {user && user.system_role === 'admin' && (
        <div className="p-4 border-t border-gray-200">
          ...
          <div className="space-y-1">
            ...
            {onAdminStatsClick && (
              <button
                onClick={onAdminStatsClick}
                className="w-full flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-gray-50 text-gray-700 transition-colors"
              >
                <svg className="w-4 h-4 flex-shrink-0 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                </svg>
                <span className="text-sm">시스템 통계</span>
              </button>
            )}
          </div>
        </div>
      ...
      <div className="p-4 border-t border-gray-200 relative" ref={menuRef}>
        {user ? (
          ...
          <button
            onClick={onLogin}
            className="w-full px-4 py-3 rounded-lg bg-gradient-to-r from-violet-600 to-purple-600 text-white font-medium hover:shadow-lg hover:shadow-violet-500/50 transition-all flex items-center justify-center space-x-2"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
